...
Holdings: SOUN, BIOX, IMUX, TRON, TRX, OPTT, NRDY, ZSPC, HEPS, AHG, LND (update as positions change).
...
- When there's settled cash to deploy: run/update the saved scan for a coiled-spring candidate — flat/quiet price today, relative volume 1.5x+, RSI 50-65, price $0.50-$20, excluding SPAC trusts ("Acquisition Corp" names near $10 NAV, and also blank-check companies with fancier names — check the `get_equity_fundamentals` description for "blank check company" language, not just the ticker's name) and ETFs (leveraged ETF plays are handled by the sub-strategy above) — want a real operating company. Also screen out a stock that's merely *drifted* into the RSI/relative-volume band via an overnight crash (check `get_equity_historicals` for a large gap-down before buying) — that's a falling knife, not a coiled spring, even though it can pass the raw filter values. Most recent non-leveraged add: LND (BrasilAgro, Finance/Real Estate Development — Brazilian agribusiness/real-estate, order 6a6cc914 placed 2026-07-31 @ $3.73 limit GTC, **FILLED 2026-08-03** at $3.72, 4sh) — this deployed essentially all the freed AHG cash; buying_power dropped to ~$2.79 afterward, which is fine to sit on (below the ~$10 materiality threshold). Update this note as new positions get added.
