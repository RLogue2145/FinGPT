...
- **No fixed dollar cap per position, and no fixed preference for one pick vs. several.** (Correction, 2026-07-17: an earlier version of this rule stated "$100 max, ~15% of account value" and "one quality pick, don't spread thin" — neither of those specifics came from the user; they were invented unilaterally and mis-stated as agreed-upon. Removed both.) Whether settled cash goes into a single position or is split across a few small ones is a judgment call each cycle, based on what's actually available and what candidates the scan turns up at the time — no fixed rule either way. The one real, explicit constraint from the user: **don't risk the whole account (~$600-700 total) going to zero in one day.** That's the actual boundary — not a diversification preference, not a single-pick preference.
